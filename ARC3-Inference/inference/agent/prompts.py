...
VISUAL_GAME_ADDENDUM = (
    ...
    "- Some games are logic or layout puzzles with no explicit player avatar or controllable sprite on the board. Do not assume a player exists; the relevant state may be an object, region, cursor, selector, or whole-board configuration.\n""- If UP/DOWN/LEFT/RIGHT (or equivalent directional keys) are among the valid actions, that IS positive evidence of a navigable avatar -- run the one-shot avatar probe below before assuming otherwise. The \"no player\" warning above is about pure click/logic puzzles that expose no directional keys at all.\n"
...
PYTHON_ADDENDUM = (
    ...
    "- IMPORTANT: Especially when the game is about making an agent navigate to a target, it is usually safer to write an explicit search algorithm such as BFS. More generally, when the objective is understood but the best action order is unclear, pathfinding, flood fill, BFS, DFS, beam search, shortest-path search, limited action-sequence search, or custom heuristics are all valid.\n""- Directional-key avatar probe (run once per game, cheap: 4 actions): press UP, DOWN, LEFT, RIGHT once each (skip any not in `valid_actions`), and after each compare `current_frame.segmentation` to the frame before it. The object whose `hash` stays the same but whose `bbox` position shifts consistently with the pressed direction is the avatar; write the direction-to-displacement mapping under `Cross-level notes:` once found.\n""- Once the avatar is known, build an occupancy grid from objects that do NOT move when the avatar does (walls/blockers), and route with a plain BFS over passable cells instead of stepping one direction at a time and re-observing.\n"
    ...
    "- A strong default loop is: summarize the board, infer the desired environment change, write a small scorer or search over candidate sequences, execute the best probe or plan with `action(...)`, then inspect again until you understand exactly what changed.\n""- Before every `action(...)` call, state in one short comment what you EXPECT to change (which object, which direction/attribute). After the call, compare `current_frame` (or the diff) to that expectation and print CONFIRMED or VIOLATED with one line of reason. A VIOLATED prediction means your model of this rule is wrong -- stop repeating that action and revise the model instead of retrying blindly.\n"
